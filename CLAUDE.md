...
25 tables, 986 columns. Full details: `schema/schema-reference.json`

### Affiliate (SDK: `ThcAffiliate`) — 2 fields
**Other:** id(PK), created_at(ts)

### BusinessInformation (SDK: `ThcBusinessInformation`) — 13 fields
...
### Contact (SDK: `ThcContact`) — 297 fields
...
**Team Information:** bsb, acc_name, team_role(enum:7 values), entity_name, pharmacy_abn, registered_for_gst(bool), date_first_logged_in(ts)
**Booked Appointments:** total_thc_margin, future_appointments_booked, last_appointment_booked_time(ts)
**Application Information:** application_date(ts), terms_conditions(bool), time_signed_terms(ts), application_status(enum:10 values)
**Application Fields:** sex(enum:Female|Intersex|Male|Prefer not to say), Intake_PHQ_2_Q1Q2, Prior_Product_Feedback(longtext), Intake_Onset_Preference(enum:Both|Fast (vape/flower)|Slow-sustained (oil/pastille)), Intake_Flower_Preference(enum:No preference|Smoking|Vaporising), Intake_Lineage_Preference(enum:Indica-dominant|No preference|Sativa-dominant), Intake_Organic_Preference(enum:No|No preference|Yes), identify_same_sex_as_at_birth(bool)
...
**Pharmacy Information:** gateway_id(longtext), pharmacy_logo(imageUrl), pharmacy_name, pharmacy_phone, pharmacy_state, dispenses_all_2, pharmacy_colour(longtext), pharmacy_suburb, pharmacy_address, dispenses_on_hold, dispenses_delivered, dispenses_in_transit, pharmacy_postal_code(address), dispenses_in_progress, dispenses_tracking_added, dispenses_to_be_confirmed
**Dr Information:** halaxy_email(email), practitioner, clinician_status(enum:9 values), link_to_tga_docs(url), practitioner_role, appointments_today, appointments_left_today, incomplete_appointments, clinician_time_slots_next_7_days
...
**Customer Contact:** Outreach_Notes(longtext), contact_comment(longtext)
**Partner Data:** owed(decimal), leads, qr_svg(json), paid_leads, total_sales(decimal), total_refunds(decimal), paypal_address, number_of_sales, number_of_refunds, total_commission_paid(decimal), active_referred_open_orders
...
**Dr Appointment Stats:** dr_appt_paid, dr_appt_count_all, Static_Timeslot_ID, dr_appt_cancelled_count, dr_appt_awaiting_payment_billed, dr_appointment_pending_completion, dr_appt_awaiting_payment_unbilled
**PATIENT Eligibility:** patient_eligibility_url(url)
**Dr Payment Stats:** dr_bill_date(ts), dr_bill_status(enum:Bill Created|Bill Sent|Run Payment), consultation_fee(currency), dr_pay_reference, dr_payments_paid(decimal), dr_payments_billed(decimal), dr_payments_pending(decimal), dr_payments_un_billed(decimal), doctor_payment_pending(currency), initial_consults_billed, pending_initial_consults, follow_up_consults_billed, pending_follow_up_consults
...
**Script information:** open_scripts, scripts_open, Dispenses_All, Scripts_Count, treatment_plan(enum:7 values), date_last_script(ts), scripts_archived, date_first_script(ts), scripts_fulfilled, Dispense_Avg_Spend(decimal), Scripts_Count_Used, Dispense_Avg_Margin(decimal), last_doctor_seen_id(FK→Contact), Scripts_Can_Dispense, Send_SMS_Scripts_Active(bool), Send_SMS_Scripts_Can_Dispense(bool)
...
**Feedback Tracking:** feedback_send_count, last_feedback_rating
...
### CouponProduct (SDK: `ThcCouponProduct`) — 3 fields
**Other:** id(PK), product_id(FK→Product), product_name
...
**Item Information:** gst(bool), type(enum:9 values), brand, expiry(ts), status(enum:In Stock|Unavailable), carrier(enum:Coconut Oil|Hemp Oil|Hemp Seed Oil|MCT Oil|Olive Oil|Other), sub_type(enum:Budget|Core|Craft|Vaporiser), dominance(enum:8 values), item_name, pack_size, retail_gst(currency), description(longtext), retail_price(currency), Cannabis_Type, quantity_unit(enum:7 values), packaging_date(ts), wholesale_price(currency), market_entry_date(ts), concession_discount(currency), estimated_date_in_stock(ts)
...
**Strength:** cbc(decimal), cbd(decimal), cbg(decimal), cbn(decimal), thc(decimal), Ratio(decimal), tga_category(enum:1|2|3|4|5), tga_schedule, strength_unit(enum:8 values), link_to_catalyst_listing(url)
...
**Stock Management:** id_2, notes(longtext), check_stock(bool), price_cents, product_name, product_type, tga_schedule_2, cannabis_type_2, tga_sas_category, brisbane_in_stock(bool), brisbane_is_active(bool), melbourne_in_stock(bool), melbourne_is_active(bool), product_size_amount, product_size_measure
...
**Other:** id(PK), contact_id(FK→Contact), credit_card_id(FK→CreditCard), balance(currency), bindex, contact_name, created_at(ts), currency, customer_note, due_date(ts), external_order_id, hidden(bool), internal_note, invoice_date(ts), last_modified_at(ts), last_recharged_at(ts), offer_data, recharge(bool), recharge_attempts, shipping_amount(currency), status(enum:8 values), subtotal(currency), tax_city, tax_city_amount(currency), tax_county, tax_county_amount(currency), tax_postal_code, tax_state, tax_state_amount(currency), template_id, total(currency), total_paid(currency), total_tax(currency), transaction_amount_original(currency), type(enum:Invoice|Refund receipt), unique_id
...
### ClinicalNote (SDK: `ThcClinicalNote`) — 22 fields
**System Information:** id(PK), halaxyid, last_note, unique_id, created_at(ts), ip_address, last_activity(ts), last_sms_sent(ts), profile_image(imageUrl), last_email_sent(ts), last_call_logged(ts), last_modified_at(ts), last_sms_received(ts), last_email_received(ts)
**Clinical Notes Information:** title, upload(json), content, author_id(FK→Contact), call_link(url), patient_id(FK→Contact), date_created(ts), appointment_id(FK→Appointment)

### Purchase (SDK: `ThcPurchase`) — 26 fields
**Other:** id(PK), affiliate_id(FK→Affiliate), contact_id(FK→Contact), coupon_code_id(FK→CouponCode), invoice_id(FK→Invoice), product_id(FK→Product), subscription_id(FK→Subscription), affiliate_name, coupon_code, coupon_name, created_at(ts), currency, description(longtext), discount(currency), last_modified_at(ts), level1_commission_amount(currency), level2_commission_amount(currency), name, package_id, price(currency), price_original(currency), quantity, sku, status(enum:7 values), total_purchase(currency), type(enum:One-time purchase|Payment plan|Subscription|Trial payment)
